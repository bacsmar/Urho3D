...
/*
   * 
   HELP -- the TBSelectDropdown angelscript implementation is not producing events! So no go yet.
   * 
    AUIInit ( "AUI/resources/default_font/vera.ttf", "Vera", 14, "AUI/Scripts/47_layout.ui.txt" );
    AView@ uiview = aui.GetFocusedView(); // get the topmost aui widget
    ALayout @lo0 = uiview.FindWidget("alayout0");
    if ( lo0 !is null ) lo0.SetLayoutConfig( "YGGPR" ); //"YGLPL" ); 
    AWidget @awid = uiview.FindWidget("check1");
    SubscribeToEvent(awid, "WidgetEvent", "HandleAuiEvent" );
    awid = uiview.FindWidget("check2");
    SubscribeToEvent(awid, "WidgetEvent", "HandleAuiEvent" );
    awid = uiview.FindWidget("check3");
    SubscribeToEvent(awid, "WidgetEvent", "HandleAuiEvent" );
    awid = uiview.FindWidget("choice1");
    SubscribeToEvent(awid, "WidgetEvent", "HandleAuiEvent" );
    awid = uiview.FindWidget("choice2");
    SubscribeToEvent(awid, "WidgetEvent", "HandleAuiEvent" );
    awid = uiview.FindWidget("choice3");
    SubscribeToEvent(awid, "WidgetEvent", "HandleAuiEvent" );
*/
...
void HandleAuiEvent(StringHash eventType, VariantMap& eventData)
{
    ...
    {
        if (widget.GetId() == "check1" ) //white
        {
            Color fg = (widget.GetValue()==1) ? BLACK : WHITE;
            Color bg = (widget.GetValue()==1) ? WHITE : BLACK;
        ...
        if (widget.GetId() == "check2" ) //SetSRGB
        {
            if (graphics.sRGBWriteSupport)
            {
                graphics.sRGB = (widget.GetValue()==1);
            }
            else
            {
        ...
        if (widget.GetId() == "check3" )  //autopass
        {
            ui.forceAutoHint = (widget.GetValue()==1);
        }
