...
void InitControls()
{
 /*   
    ...
    */
    ...
    AWidget @checkplease = win1.GetWidget("aCheck");
    SubscribeToEvent( checkplease, "WidgetEvent", "HandleCheckEvent" );
...
void HandleCheckEvent(StringHash eventType, VariantMap& eventData)
{
    ...
    {
            AWindow @win = cast<AWindow@>(widget.FindWidget("myWindow"));
            if (win !is null)
                win.SetText("Hello Checkbox");
    }
}
...
void HandleButtonEvent(StringHash eventType, VariantMap& eventData)
{
    ...
    if (eventData["Type"] == UI_EVENT_TYPE_CLICK)
    {
            AWindow @win = cast<AWindow@>(widget.FindWidget("myWindow"));
            if (win !is null)
                win.SetText("Hello Button");
    }
}

void HandleEditEvent(StringHash eventType, VariantMap& eventData)
{
    AWidget @widget = eventData["Target"].GetPtr();
    if ( widget is null ) return;
   // if (eventData["Type"] == UI_EVENT_TYPE_CLICK)  on ANY event!
    {
            AWindow @win = cast<AWindow@>(widget.FindWidget("myWindow"));
            if (win !is null)
                win.SetText("Hello Editfield");
    }
}
