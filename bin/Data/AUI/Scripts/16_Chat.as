...
void SubscribeToEvents()
{
    ...
    SubscribeToEvent( atextEdit, "WidgetEvent", "HandleSend" ); //SubscribeToEvent(textEdit, "TextFinished", "HandleSend");
    SubscribeToEvent( asendButton, "WidgetEvent", "HandleSend" ); //SubscribeToEvent(sendButton, "Released", "HandleSend");
    SubscribeToEvent( aconnectButton, "WidgetEvent", "HandleConnect" ); //SubscribeToEvent(connectButton, "Released", "HandleConnect");
    SubscribeToEvent( adisconnectButton, "WidgetEvent", "HandleDisconnect" ); // SubscribeToEvent(disconnectButton, "Released", "HandleDisconnect");
...
void ShowChatText(const String& row)
{
    ...
    achatHistoryText.SetText(allRows); // chatHistoryText.text = allRows;
}
...
void UpdateButtons()
{
    ...
    if (serverConnection !is null ) //sendButton.visible = serverConnection !is null;
        asendButton.SetState(UI_WIDGET_STATE_DISABLED, false);
    else asendButton.SetState(UI_WIDGET_STATE_DISABLED, true);
    
    if (serverConnection is null && !serverRunning) //connectButton.visible = serverConnection is null && !serverRunning;
        aconnectButton.SetState(UI_WIDGET_STATE_DISABLED, false);
    else aconnectButton.SetState(UI_WIDGET_STATE_DISABLED, true);
    
    if (serverConnection !is null || serverRunning) //disconnectButton.visible = serverConnection !is null || serverRunning;
        adisconnectButton.SetState(UI_WIDGET_STATE_DISABLED, false);
    else adisconnectButton.SetState(UI_WIDGET_STATE_DISABLED, true);

    if (serverConnection is null && !serverRunning) //startServerButton.visible = serverConnection is null && !serverRunning;
        astartServerButton.SetState(UI_WIDGET_STATE_DISABLED, false);
    else astartServerButton.SetState(UI_WIDGET_STATE_DISABLED, true);
}
...
void HandleSend(StringHash eventType, VariantMap& eventData)
{
    ...
    if (eventData["Type"] == UI_EVENT_TYPE_CLICK ) // || eventData["Type"] == UI_EVENT_TYPE_CHANGED )
    {

        String text = atextEdit.GetText(); //textEdit.text;
        if (text.empty)
        ...
        if (serverConnection !is null)
        {
            ...
            atextEdit.SetText(""); //textEdit.text = "";
        }
    }
}
...
{
   String address = atextEdit.GetText().Trimmed(); //textEdit.text.Trimmed();
    if (address.empty)
