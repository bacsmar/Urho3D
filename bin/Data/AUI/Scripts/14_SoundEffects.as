...
*/
    ...
    AWidget @button = uiview.FindWidget("fistbutton");
    ...
    AWidget @slider = uiview.FindWidget("soundvolume");
...
void HandlePlaySound(StringHash eventType, VariantMap& eventData)
{
    ...
    Sound@ sound = null; //cache.GetResource("Sound", soundResourceName);

    AWidget @widget = eventData["Target"].GetPtr();
    if ( widget is null ) return;
    if (eventData["Type"] == UI_EVENT_TYPE_CLICK)
    {
        if (widget.GetId() == "fistbutton" )
        {
            sound = cache.GetResource("Sound", soundResourceNames[0]);
        }
        if (widget.GetId() ==  "explosionbutton" )
        {
            sound = cache.GetResource("Sound", soundResourceNames[1]);
       }
        if (widget.GetId() == "powerupbutton" )
        {
...
void HandleSoundVolume(StringHash eventType, VariantMap& eventData)
{
    ...
    {
        float newVolume = widget.GetValue();  //eventData["Value"].GetFloat();
