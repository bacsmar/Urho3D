...
#include "AUI/Scripts/Utilities/2D/Mover.as"
...
void HandlePlayButton(StringHash eventType, VariantMap& eventData)
{
    ...
    {
        AView@ uiview = aui.GetFocusedView(); // get the topmost aui widget
        AWidget @awid = uiview.FindWidget("FullUI");

       // Remove fullscreen UI and unfreeze the scene
        if ( awid !is null )
        {
            AWidget @ab = uiview.FindWidget("gameplay");
            UnsubscribeFromEvent(ab, "WidgetEvent"); // unsubscribe or it will keep firing!
