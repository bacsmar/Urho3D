...
void TBWidget::InvokeRightPointerDown(int x, int y, int click_count, MODIFIER_KEYS modifierkeys)
{
    ...
    TBWidget* widget = GetWidgetAt(x, y, true);

    if (!widget)
        return;

    TBWidgetEvent ev(EVENT_TYPE_RIGHT_POINTER_DOWN, x, y, false, modifierkeys);
    ev.count = click_count;
    widget->InvokeEvent(ev);
}

void TBWidget::InvokeRightPointerUp(int x, int y, MODIFIER_KEYS modifierkeys)
{
    // note we're not capturing the widget as with normal pointer down
    TBWidget* widget = GetWidgetAt(x, y, true);
    ...
    TBWidgetEvent ev(EVENT_TYPE_RIGHT_POINTER_UP, x, y, false, modifierkeys);
    ev.target = widget;
    widget->InvokeEvent(ev);
...
bool TBWidget::InvokeKey(int key, SPECIAL_KEY special_key, MODIFIER_KEYS modifierkeys, bool down)
{
    ...
    if (focused_widget)
    {
        // ATOMIC: This may have some side effects for shortcuts we don't want?
        if (focused_widget->GetDisabled())
            return false;
