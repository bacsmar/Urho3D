...
void TBSelection::RemoveContent()
{
    ...
    if (styledit->text_change_listener)
        styledit->text_change_listener->OnChange(styledit);
...
int32 TBBlock::FirstNonTabPos() const
{
...
void TBBlock::Layout(bool update_fragments, bool propagate_height)
{
    ...
    while (first_fragment_on_line)
    {
        ...
        while (fragment)
        {
            line_height = TBMAX(fragment->GetHeight(styledit->font), line_height);
            line_baseline = TBMAX(fragment->GetBaseline(styledit->font), line_baseline);
        ...
        while (fragment)
        {
            ...
            adjusted_line_height = TBMAX(line_baseline - fragment->GetBaseline(styledit->font) + fragment->GetHeight(styledit->font), adjusted_line_height);
        ...
        line_width_max = TBMAX(line_width_max, line_width);
                                          ...
                                          TBRegion &bg_region, TBRegion &fg_region)
{
    ...
    sofs1 = TBMAX(sofs1, (int)ofs);
    sofs2 = TBMIN(sofs2, (int)(ofs + len));
...
void TBTextFragment::Paint(int32 translate_x, int32 translate_y, TBTextProps *props)
{
    ...
    if (block->styledit->text_theme)
        color = block->styledit->text_theme->themeColors[themeColor];
...
{
    x = TBMIN(x, GetContentWidth() - layout_width);
    y = TBMIN(y, GetContentHeight() - layout_height);
    x = TBMAX(x, 0);
    y = TBMAX(y, 0);
...
int32 TBStyleEdit::GetContentWidth()
{
    if (packed.calculate_content_width_needed)
    {
        ...
        while (block)
        {
            content_width = TBMAX(content_width, block->line_width_max);
            block = block->GetNext();
        }
    }
