...
{
    float abs_start_speed = TBABS(start_speed);
...
void TBScroller::OnPanReleased()
{
    if (TBSystem::GetTimeMS() < m_pan_time_ms + PAN_START_THRESHOLD_MS)
    {
        ...
        if (TBABS(m_pan_dx) < pan_start_distance_threshold_px && TBABS(m_pan_dy) < pan_start_distance_threshold_px)
        {
            StopOrSnapScroll();
            return;
        }
...
void TBScroller::OnMessageReceived(TBMessage *msg)
{
    if (msg->message == TBIDC("scroll"))
    {
        ...
        elapsed_time_x = TBMIN(elapsed_time_x, m_scroll_duration_x_ms);
        elapsed_time_y = TBMIN(elapsed_time_y, m_scroll_duration_y_ms);
        ...
        if (!StopIfAlmostStill())
        {
            ...
            next_fire_time = TBMAX(next_fire_time, now_ms);
