...
{
    overflow_scroll = TBMIN(overflow_scroll, m_overflow);
    overflow_scroll = TBMAX(overflow_scroll, 0);
...
int TBLayout::GetWantedHeight(WIDGET_GRAVITY gravity, const PreferredSize &ps, int available_height) const
{
    ...
    case LAYOUT_SIZE_GRAVITY:
        height = ((gravity & WIDGET_GRAVITY_TOP) && (gravity & WIDGET_GRAVITY_BOTTOM)) ?
                    available_height : TBMIN(available_height, ps.pref_h);
        break;
    case LAYOUT_SIZE_PREFERRED:
        height = TBMIN(available_height, ps.pref_h);
        break;
    case LAYOUT_SIZE_AVAILABLE:
        height = TBMIN(available_height, ps.max_h);
        break;
    };
    height = TBMIN(height, ps.max_h);
    return height;
}
...
void TBLayout::ValidateLayout(const SizeConstraints &constraints, PreferredSize *calculate_ps)
{
    ...
    for (TBWidget *child = GetFirstInLayoutOrder(); child; child = GetNextInLayoutOrder(child))
    {
        ...
        {
            int capped_max_w = TBMIN(layout_rect.w, ps.max_w);
            total_max_pref_diff_w += capped_max_w - ps.pref_w;
        }

        if (calculate_ps)
        {
            calculate_ps->min_h = TBMAX(calculate_ps->min_h, ps.min_h);
            calculate_ps->pref_h = TBMAX(calculate_ps->pref_h, ps.pref_h);
            ...
            calculate_ps->max_h = TBMAX(calculate_ps->max_h, height);
    ...
    int missing_space = TBMAX(total_preferred_w - layout_rect.w, 0);
    int extra_space = TBMAX(layout_rect.w - total_preferred_w, 0);
    ...
    if (extra_space && m_packed.layout_mode_dist_pos != LAYOUT_DISTRIBUTION_POSITION_LEFT_TOP)
    {
        ...
        if (m_packed.layout_mode_dist != LAYOUT_DISTRIBUTION_PREFERRED)
            used_space += TBMIN(extra_space, total_max_pref_diff_w);
    ...
    m_overflow = TBMAX(0, used_space - layout_rect.w);
...
TBStr TBLayout::GetLayoutConfig()  /// return a string that represents the current layout settings
{
	TBStr mysettings;

        switch ( m_axis ) // encode AXIS
        {
            case AXIS_X: mysettings.Append("X"); break;
            case AXIS_Y: mysettings.Append("Y"); break;
            default: mysettings.Append("-"); break;
        }
        switch ( m_packed.layout_mode_size  ) // SIZE
        {
            case LAYOUT_SIZE_AVAILABLE: mysettings.Append("A"); break;
            case LAYOUT_SIZE_GRAVITY: mysettings.Append("G"); break;
            case LAYOUT_SIZE_PREFERRED: mysettings.Append("P"); break;
            default: mysettings.Append( "-"); break;
        }
        switch ( m_packed.layout_mode_pos ) // POSITION
        {
            case LAYOUT_POSITION_CENTER: mysettings.Append("C"); break;
            case LAYOUT_POSITION_GRAVITY: mysettings.Append("G"); break;
            case LAYOUT_POSITION_LEFT_TOP: mysettings.Append("L"); break;
            case LAYOUT_POSITION_RIGHT_BOTTOM: mysettings.Append("R"); break;
            default: mysettings.Append( "-"); break;
        }
        switch ( m_packed.layout_mode_dist  ) // DISTRIBUTION
        {
            case LAYOUT_DISTRIBUTION_AVAILABLE: mysettings.Append("A"); break;
            case LAYOUT_DISTRIBUTION_GRAVITY: mysettings.Append("G"); break;
            case LAYOUT_DISTRIBUTION_PREFERRED: mysettings.Append("P"); break;
            default: mysettings.Append( "-"); break;
        }
        switch ( m_packed.layout_mode_dist_pos ) // DISTRIBUTION_POSITION
        {
            case LAYOUT_DISTRIBUTION_POSITION_CENTER: mysettings.Append("C"); break;
            case LAYOUT_DISTRIBUTION_POSITION_LEFT_TOP: mysettings.Append("L"); break;
            case LAYOUT_DISTRIBUTION_POSITION_RIGHT_BOTTOM: mysettings.Append("R"); break;
            default: mysettings.Append("-"); break;
        }
